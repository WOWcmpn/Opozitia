    ...
    (() => {
        ...
        class Popup {
            ...
            _openToHash() {
                ...
                const buttons = document.querySelector(`[${this.options.attributeOpenButton} = "${classInHash}"]`) ? document.querySelector(`[${this.options.attributeOpenButton} = "${classInHash}"]`) : "";
        ...
        function initSliders() {
            ...
            if (document.querySelector(".graphics-left-block__slider")) new core(".graphics-left-block__slider", {
            ...
            if (document.querySelector(".graphics-left-block__slider2")) new core(".graphics-left-block__slider2", {
                modules: [ Autoplay, EffectFade ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
            ...
            if (document.querySelector(".bottom-left-block__slider")) new core(".bottom-left-block__slider", {
                modules: [ Autoplay, EffectFade ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
            ...
            if (document.querySelector(".bottom-left-block__slider2")) new core(".bottom-left-block__slider2", {
                modules: [ Autoplay, EffectFade ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
            ...
            if (document.querySelector(".bottom-left-block__slider3")) new core(".bottom-left-block__slider3", {
                modules: [ Autoplay, EffectFade ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
            ...
            if (document.querySelector(".thumbs-images")) {
                ...
                new core(".bottom-left-block__slider-main", {
                    modules: [ Autoplay, Thumb, EffectFade ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 1,
                    watchOverflow: true,
            ...
            if (document.querySelector(".championship__slider")) new core(".championship__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
                ...
                speed: 200,
                navigation: {
                    prevEl: ".championship__swiper-button-prev",
                    nextEl: ".championship__swiper-button-next"
                },
                on: {}
            });
        }
        ...
        function bannerTop() {
            let banner = document.querySelector(".help");
            let btnClose = document.querySelector(".help__btn");
            let timerId;
            if (localStorage.getItem("banner") === "hidden") ; else timerId = setInterval((() => banner.classList.toggle("active")), 4e3);
        ...
        document.querySelector(".body-popup__wrap-input").addEventListener("input", (function(e) {
            e.target.value = e.target.value.replace(/[^0-9]/g, "");
            if (e.target.value !== "" && e.target.nextElementSibling && e.target.nextElementSibling.nodeName === "INPUT") e.target.nextElementSibling.focus();
        }));
        function checkPass() {
            let pass1 = document.getElementById("pass");
            let pass2 = document.getElementById("pass2");
            let formPass = document.getElementById("form-pass");
            formPass.addEventListener("submit", (function() {
                if (pass1.value != pass2.value) {
                    alert("Пароли не совпадают");
                    formPass.setAttribute("data-popup-message", "");
                    return false;
                } else {
                    formPass.setAttribute("data-popup-message", "#popup-registration-recovery");
                    return true;
                }
            }));
        }
        checkPass();
        const oprosiConainer = document.querySelector(".oprosi");
        if (oprosiConainer) {
            ...
            for (let i = 0; i < options.length; i++) options[i].addEventListener("click", (() => {
                ...
                while (j < options.length) {
                    if (options[j].classList.contains("selected")) options[j].classList.remove("selected");
                    j++;
                }
                options[i].classList.add("selected");
                flsModules.popup.open("#popup-vote");
                options[i].parentElement.classList.add("selected");
                for (let k = 0; k < options.length; k++) options[k].classList.add("selectall");
                let forVal = options[i].getAttribute("for");
                document.querySelector("#" + forVal);
                let array = [];
                for (let l = 0; l < options.length; l++) if (options[l].classList.contains("selected")) array.push(l);
                if (array.length == 0) for (let m = 0; m < options.length; m++) options[m].removeAttribute("class");
            }));
        }
        const btnMore = document.querySelector(".btn-more");
        const itemsForMore = document.querySelectorAll(".item-block.hidden");
        if (btnMore) btnMore.addEventListener("click", (() => {
            if (itemsForMore) for (let i = 0; i < itemsForMore.length; i++) {
                itemsForMore[i].classList.remove("hidden");
                btnMore.classList.add("hidden");
            }
        }));
        const shareContainer = document.querySelector(".main-block-news__actions-list");
        const sortContainer = document.querySelector(".top-comments-news__list");
        function shareOpen() {
            document.addEventListener("click", (function(e) {
                if (shareContainer) if (e.target.closest(".main-block-news__actions")) shareContainer.classList.toggle("active"); else shareContainer.classList.remove("active");
                if (sortContainer) if (e.target.closest(".top-comments-news__btn-sort")) sortContainer.classList.toggle("active"); else sortContainer.classList.remove("active");
            }));
        }
        shareOpen();
        const currentDate = document.querySelector(".calendar-vidgets__current-date");
        const daysTag = document.querySelector(".calendar-vidgets__days");
        const prevNextIcon = document.querySelectorAll(".calendar-vidgets__icons span");
        let date = new Date, currYear = date.getFullYear(), currMonth = date.getMonth(), weekDay = date.getDay(), monthDay = date.getDate(), month = date.getMonth();
        ...
        const renderCalendar = () => {
            ...
            let countMonthDay;
            if (weekDay > 1) countMonthDay = monthDay - (weekDay - 1); else if (weekDay === 0) countMonthDay = monthDay - 6; else countMonthDay = monthDay;
            const results = [];
            function resultsAdd() {
                for (let k = 0; k < week.length; k++) if (countMonthDay + k > countDayOnMonth[month]) {
                    let count = countDayOnMonth[month] - (countMonthDay + (week.length - 1));
                    results.push([ `${count + k}` ]);
                } else results.push([ `${countMonthDay + k}` ]);
                return;
            }
            resultsAdd();
            for (let i = 1; i <= lastDateofMonth; i++) {
                countDayOnMonth[month], week.length;
                date.getDate();
                if (i === date.getDate() && currMonth === (new Date).getMonth() && currYear === (new Date).getFullYear()) liTag += `<li class="active">${i}</li>`; else liTag += `<li class="">${i}</li>`;
            }
        ...
        function sethrefs() {
            if (document.querySelectorAll) {
                var datahrefs = document.querySelectorAll("[data-href]"), dhcount = datahrefs.length;
                while (dhcount-- > 0) {
                    var ele = datahrefs[dhcount], addevent = function(ele, event, func) {
                        if (ele.addEventListener) ele.addEventListener(event, link, false); else ele.attachEvent("on" + event, link);
                    }, link = function(event) {
                        var target = event.target, url = this.getAttribute("data-href");
                        if (!target.href) {
        ...
        function getWeather() {
            ...
            const weatherIcons = document.querySelectorAll(".weather-header__icon");
            let location = document.getElementById("location");
            let api = "https://api.openweathermap.org/data/2.5/weather";
            let apiKey = "b37913b4e13eb52a97731b364eddcb6a";
            location.innerHTML = "Загрузка...";
            navigator.geolocation.getCurrentPosition(success, error);
            function success(position) {
                let latitude = position.coords.latitude;
                let longitude = position.coords.longitude;
                let url = api + "?lat=" + latitude + "&lon=" + longitude + "&appid=" + apiKey + "&lang=ru" + "&units=metric";
                fetch(url).then((response => response.json())).then((data => {
                    console.log(data);
                    let temp = data.main.temp;
                    temperature.innerHTML = Math.round(temp) + "° C";
                    location.innerHTML = data.name;
                    weatherIcons.forEach((function(item, i, arr) {
                        let url = "http://openweathermap.org/img/w/" + data.weather[0].icon + ".png";
                        weatherIcons[i].src = url;
                    }));
                }));
            }
            function error() {
                location.innerHTML = "Не удалось получить ваше местоположение";
            }
        }
        document.addEventListener("DOMContentLoaded", getWeather);
        document.addEventListener("DOMContentLoaded", bannerTop);
        let mybutton = document.querySelector(".page__icon-top");
        if (mybutton) window.onscroll = function() {
            scrollFunction();
        };
        function scrollFunction() {
            if (document.body.scrollTop > 20 || document.documentElement.scrollTop > 20) mybutton.classList.add("show"); else mybutton.classList.remove("show");
        }
